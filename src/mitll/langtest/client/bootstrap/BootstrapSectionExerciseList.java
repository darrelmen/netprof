 ...
 */
public class BootstrapSectionExerciseList extends PagingExerciseList {
  public static final String ANY = "Any";
  private Container sectionPanel;
  private long userID;
...
/*  @Override
  ...
  private void populateListBox(String type, Collection<String> sections) {
    ...
    System.out.println("populateListBox for " +type); // TODO : fill in
  /*  populateListBox(listBox, sections);
...
/*  private Widget getEmailWidget() {
  ...
  private void pushFirstSelection(String first) {
    ...
    if (initToken.length() == 0) {
     // SelList listBox = typeToBox.get(first);
...
/*    service.getExercisesForSection(type, section, new SetExercisesCallback() {
  ...
  protected String getHistoryToken(String id) {
    ...
    for (String type : typeToBox.keySet()) {
      SelList listBox = typeToBox.get(type);
      String section = listBox.getCurrentValue();// listBox.getItemText(listBox.getSelectedIndex());
  ...
  private String getDefaultToken() {
    ...
    for (String type : typeToBox.keySet()) {
      SelList listBox = typeToBox.get(type);
      String section = listBox.getCurrentValue();
      if (section.equals(ANY)) {
        // ?
      }
      else {
