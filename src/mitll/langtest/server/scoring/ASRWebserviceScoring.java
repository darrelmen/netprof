...
public class ASRWebserviceScoring extends Scoring implements CollationSort {
  ...
  private PretestScore scoreRepeatExercise(String testAudioDir,
    ...
    Scores scores = runHydra(rawAudioPath, sentence, tmpDir, decode);
    if (scores == null) {
  ...
  //===================================
    private Scores getScoreForAudio_HYDRA(String testAudioDir, String testAudioFileNoSuffix,
                                  String sentence,
                                  String scoringDir,
                                  boolean decode, String tmpDir, boolean useCache) {
    String key = testAudioDir + File.separator + testAudioFileNoSuffix;
  ...
  //===================================
  /*private Scores calcScoreForAudio_HYDRA(String testAudioDir, String testAudioFileNoSuffix,
                                   String sentence,
                                   String scoringDir,
                                   boolean decode, String tmpDir) {

    // TODO what is this doing and how do I remove pronz?
    Dirs dirs = pronz.dirs.Dirs$.MODULE$.apply(tmpDir, "", scoringDir, new Log(null, true));
    Audio testAudio = Audio$.MODULE$.apply(
      testAudioDir, testAudioFileNoSuffix,
      false, dirs);
  ...
  //===================================
  private Scores computeRepeatExerciseScores_HYDRA(Audio testAudio, String sentence, String tmpDir, boolean decode) {

    Scores scoresFromHydra = getScoresFromHydra(testAudio, sentence);

    double hydraScore = scoresFromHydra.hydraScore;
    if (hydecScore > lowScoreThresholdKeepTempDir) {   // keep really bad scores for now
      logger.debug("JESS: Not removing tmp dir currently: " + tmpDir)
      //      try {
      //        FileUtils.deleteDirectory(new File(tmpDir));
      //      } catch (IOException e) {
      //        logger.error("Deleting dir " + tmpDir + " got " +e,e);
  ...
  private Scores runHydra(String audioPath, String transcript, String tmpDir, boolean decode, ArrayList<String> dictWords, ArrayList<String> smallLM) {
    // reference trans
    String cleaned = transcript.replaceAll("\\u2022", " ").replaceAll("\\p{Z}+", " ").replaceAll(";", " ").replaceAll("~", " ").replaceAll("\\u2191", " ").replaceAll("\\u2193", " ");
    String hydraInput = makeHydraInput(decode, audioPath, cleaned, dictWords, smallLM, tmpDir);

    String scores = langTestDatabase.getHTTPClient().sendAndReceive(hydraInput);
    if(scores == "") {
      logger.error("Failure during running of hydra.");
      return null;
    }
    return new Scores(scores); // TODO write this constructor
  }
  
  // NOTE dictionary is only the words that we need
  // NOTE dictionary needs "sp" at the end of the pronunciations
  // NOTE dictionary values are the prons. should be strings of space separated phones 
  // this function builds the initial input into hydra, so it is just a single segment
  private String makeHydraInput(boolean decode, String audioPath, String transcript, Map<String, ArrayList<String>> dictWords, ArrayList<String> smallLM, String tmpDir) {
    String input = tmpDir + "/:" + audioPath + ":[";
    // format dictionary
    for(String key : dictWords.keys()) {
      ArrayList<String> prons = dictWords.get(key);
    ...
    input += "]:xxx,-1,-1,[" + transcript.replaceAll("\\p{Z}+", ";") + "]";
    logger.debug("INITIAL HYDRA INPUT: " + input);
    return input;
  }

  // OLD OLD OLD OLD
  //===================================
  /*  private Scores getScoresFromHydra(Audio testAudio, String sentence, String configFile) {
    // trans reference
    sentence = sentence.replaceAll("\\u2022", " ").replaceAll("\\p{Z}+", " ").replaceAll(";", " ").replaceAll("~", " ").replaceAll("\\u2191", " ").replaceAll("\\u2193", " ");

    long then = System.currentTimeMillis();

    logger.debug("getScoresFromHydra scoring '" + sentence +"' (" +sentence.length()+ " ) with LTS " + letterToSoundClass);
