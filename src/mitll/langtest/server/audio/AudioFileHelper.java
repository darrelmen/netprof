...
public class AudioFileHelper implements CollationSort, AlignDecode {
  ...
  public AudioAnswer writeAudioFile(String base64EncodedString,
                                    int reqid, CommonExercise exercise1,

//                                    int user,
//                                    String exerciseID,
//                                    int questionID,
//                                    String audioType,
                                    AnswerInfo.AudioContext audioContext,
...
//                                    String deviceType, String device,
   ...
   // AnswerInfo.AudioContext audioContext = new AnswerInfo.AudioContext(user, exerciseID, 0, audioType);
    ...
    return getAudioAnswerDecoding(reqid, exercise1,
...
//        recordedWithFlash, deviceType, device,
   ...
   */
  public AudioAnswer getAnswer(int reqid,
...
//            (serverProps.isQuietAudioOK() && validity.getValidity() == AudioAnswer.Validity.TOO_QUIET);
    ...
    AnswerInfo.AudioContext audioContext = new AnswerInfo.AudioContext(user, exerciseID, 0, doFlashcard ? "flashcard" : "learn");
    AnswerInfo.RecordingInfo recordingInfo = new AnswerInfo.RecordingInfo("", file.getPath(), deviceType, device, true);

    return doFlashcard ?
        getAudioAnswerDecoding(reqid, exercise1,
            audioContext, recordingInfo,
            wavPath, file,
            //isValid,
            validity, true, true, allowAlternates,
            false) :
        getAudioAnswerAlignment(reqid, exercise1,
            audioContext, recordingInfo,
            wavPath, file,
            //isValid,
            validity,
   ...
   */
  private AudioAnswer getAudioAnswerDecoding(int reqid,
                                             CommonExercise exercise1,

                                             AnswerInfo.AudioContext context,
    ...
    AudioAnswer answer = getAudioAnswer(reqid, exercise1, doFlashcard, wavPath, file, validity,
        true,
        allowAlternates, useOldSchool);
 ...
 /*     long answerID = db.addAudioAnswer(user, exerciseID, questionID, file.getPath(),
   ...
   * @param useOldSchool
   * @return
   * @see #getAnswer(int, CommonExercise, String, String, File, String, String, float, boolean, boolean)
   */
  private AudioAnswer getAudioAnswerAlignment(int reqid,
                                              ...
                                              boolean useOldSchool) {
    logValidity(context.getId(), context.getQuestionID(), context.getUserid(), file, validity);
    AudioAnswer answer = getAudioAnswer(reqid, exercise1, doFlashcard, wavPath, file, validity, true, false, useOldSchool);
...
/*
      AnswerInfo info = new AnswerInfo(user,
          exerciseID,
          questionID,
          audioType, "",
          file.getPath(),
          deviceType, device, recordedWithFlash, validity.durationInMillis, isValid,
          validity.getValidity().name(), validity.getMaxMinRange(), true,
          score,
          new ScoreToJSON().getJsonFromAnswer(answer).toString(),
          processDur
          //   0,
      );
*/
      ...
      long answerID = db.getAnswerDAO().addAnswer(db, info);
/*      user, exerciseID, questionID, file.getPath(),
