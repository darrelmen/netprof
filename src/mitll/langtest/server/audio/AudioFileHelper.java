...
public class AudioFileHelper {
  ...
  public AudioAnswer writeAudioFile(String base64EncodedString, String plan, String exerciseID, CommonExercise exercise1, int questionID,
    ...
    return getAudioAnswer(plan, exerciseID, exercise1, questionID, user, reqid, flq, audioType, doFlashcard, recordInResults, recordedWithFlash, wavPath, file, validity, isValid);
  }

  public AudioAnswer getAnswer(String exerciseID, CommonExercise exercise1, int user, boolean doFlashcard, String wavPath, File file, String deviceType, String device, float score) {
    String audioType = doFlashcard ? "flashcard" : "learn";
    AudioCheck.ValidityAndDur validity = new AudioConversion().isValid(file);
    boolean isValid = validity.validity == AudioAnswer.Validity.OK || (serverProps.isQuietAudioOK() && validity.validity == AudioAnswer.Validity.TOO_QUIET);
    return doFlashcard ? getAudioAnswer("plan", exerciseID, exercise1, 0, user, 0, true, audioType, doFlashcard, true, true, wavPath, file, validity, isValid) :
        getAudioAnswer("plan", exerciseID, exercise1, 0, user, 0, true, audioType, doFlashcard, true, true, wavPath, file, validity, isValid, score)
        ;
  }

  private AudioAnswer getAudioAnswer(String plan, String exerciseID, CommonExercise exercise1, int questionID, int user,
                                     int reqid, boolean flq, String audioType, boolean doFlashcard, boolean recordInResults,
                                     boolean recordedWithFlash, String wavPath, File file, AudioCheck.ValidityAndDur validity, boolean isValid) {
    if (!isValid) {
  ...
  private AudioAnswer getAudioAnswer(String plan, String exerciseID, CommonExercise exercise1, int questionID, int user,
    ...
    if (recordInResults) {
      long answerID = db.addAudioAnswer(user, plan, exerciseID, questionID, file.getPath(),
          isValid, flq, true, audioType, validity.durationInMillis, true, score, recordedWithFlash);
      answer.setResultID(answerID);
    }
    logger.debug("writeAudioFile answer " + answer);
    return answer;
